...
import com.thgiang.image.core.util.throttleLatest
import com.thgiang.image.admin.repository.TemplateExportManager
import com.thgiang.image.admin.repository.TemplateFileRepository
...
class TemplateBuilderViewModel @Inject constructor(
    ...
    private val exportManager: TemplateExportManager,
    ...
    // I/O operations delegated to TemplateFileRepository

    private fun CloudLayer.resolvedImageUrl(): String? {
    ...
    private fun loadCloudTemplate(cloudTemplate: CloudTemplate) {
        ...
        templateLoadJob = viewModelScope.launch(Dispatchers.IO) {
            try {
                fileRepository.getInputStreamForPath(assetPath)?.use { input ->
                ...
                _state.update { it.copy(errorMessage = "Không thể tải template JSON: ${e.message}") }
            }
        }
    }
    ...
    private fun setProductImage(uri: Uri, replaceLayerId: String?, isSample: Boolean = false) {
        bgRemoveJob?.cancel()
        
        val processingId = replaceLayerId ?: java.util.UUID.randomUUID().toString()
        val oldLayer = _state.value.layers.find { it.id == replaceLayerId }
        val targetIsSample = oldLayer?.product?.isSample ?: isSample
    ...
    fun setCustomBackground(uri: Uri) {
        viewModelScope.launch {
            loadTemplate(uri.toString())
        }
    }

    fun addSampleObject(uri: Uri) {
        setProductImage(uri, null, isSample = true)
    }

    fun addDecoration(uri: Uri) {
        val processingId = java.util.UUID.randomUUID().toString()
        _state.update { state ->
        ...
        viewModelScope.launch {
            try {
                ...
                _state.update { state ->
                    val newProduct = EditorProduct(
                        originalUriString = uri.toString(),
                        foregroundUriString = uri.toString(),
                        isBackgroundRemoved = true,
                        baseWidth = baseSize.width,
                        baseHeight = baseSize.height,
                        processing = false,
                ...
                android.util.Log.e(TAG, "Failed to load decoration: $uri", e)
                _state.update { state ->
                    state.copy(
                        layers = state.layers.filterNot { it.id == processingId },
                        selectedLayerId = if (state.selectedLayerId == processingId) null else state.selectedLayerId,
    ...
    // ============ Export (delegated to TemplateExportManager) ============

    fun exportToJson(title: String, categoryId: String) {
        val currentState = _state.value
        val validationErrors = exportManager.validateForExport(
            title, categoryId, currentState.template, currentState.layers
        )
        if (validationErrors.isNotEmpty()) {
            _state.update { it.copy(errorMessage = validationErrors.joinToString("\n")) }
            return
        }

        viewModelScope.launch {
            exportManager.exportToJson(title, categoryId, currentState.template, currentState.layers)
                .onSuccess { file ->
                    _state.update { it.copy(errorMessage = "Đã lưu tại:\n${file.name}") }
                }
                .onFailure { e ->
                    android.util.Log.e(TAG, "Export JSON failed", e)
                    _state.update { it.copy(errorMessage = "Lỗi xuất JSON: ${e.message}") }
                }
        }
    }

    fun exportToBundle(title: String, categoryId: String) {
        val currentState = _state.value
        val validationErrors = exportManager.validateForExport(
            title, categoryId, currentState.template, currentState.layers
        )
        ...
        viewModelScope.launch {
            exportManager.exportToBundle(title, categoryId, currentState.template, currentState.layers)
                .onSuccess { result ->
                    _state.update { it.copy(errorMessage = "Da luu bundle:\n${result.bundleName}\n${result.zipFileName}") }
                }
                .onFailure { e ->
                    android.util.Log.e(TAG, "Export bundle failed", e)
                    _state.update { it.copy(errorMessage = "Loi xuat bundle: ${e.message}") }
                }
        }
    }
