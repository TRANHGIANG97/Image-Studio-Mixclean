      ...
      <div className="space-y-8 animate-in fade-in duration-500">
        <div className="p-8 rounded-3xl bg-slate-900 border border-slate-800 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <h1 className="text-2xl font-bold tracking-tight text-slate-100">Cấu hình Cơ sở dữ liệu</h1>
          </div>
          ...
          <div className="space-y-4 max-w-3xl">
            <div className="p-5 rounded-2xl bg-slate-950 border border-slate-800">
              ...
              <p className="text-xs text-slate-400 mb-3">
                Hãy tạo một file tên <code className="text-indigo-400">.env.local</code> ở thư mục gốc <code className="text-indigo-400">admin_web/</code> của bạn (dựa trên file mẫu <code className="text-slate-300">.env.local.example</code>) và điền các API key của bạn vào.
              </p>
              <pre className="text-xs bg-slate-900 p-4 rounded-xl text-slate-300 font-mono overflow-x-auto border border-slate-800">
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">Chào mừng quay trở lại, hệ thống vận hành hoàn toàn ổn định.</p>
      </div>
        ...
        {stats.map((stat) => {
          ...
          return (
            <Link key={stat.name} href={stat.href} className="group">
              <div className={`p-6 rounded-3xl bg-slate-900 border border-slate-800 shadow-lg ${stat.shadow} transition-all duration-300 hover:border-slate-700 hover:translate-y-[-2px] relative overflow-hidden flex items-center justify-between`}>
      ...
      <div className="p-6 rounded-3xl bg-slate-900 border border-slate-800 shadow-xl">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-slate-200">Templates Cập nhật Gần nhất</h2>
          </div>
          <Link href="/templates">
        ...
        {data.recentTemplates.length === 0 ? (
          <div className="text-center py-12 border border-dashed border-slate-800 rounded-2xl">
            ...
            <p className="text-slate-400 font-medium">Chưa có Template nào được tạo.</p>
            <p className="text-xs text-slate-500 mt-1 mb-4">Hãy tạo template mới để thiết kế ngay hôm nay.</p>
            <Link href="/templates">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {data.recentTemplates.map((tpl) => (
                  <tr key={tpl.id} className="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors group">
                    ...
                    <td className="py-4 text-sm text-slate-300">{(tpl.categories as any)?.name || 'Chưa phân loại'}</td>
                    <td className="py-4">
                    ...
                    <td className="py-4 text-right text-xs text-slate-400 pr-4">
