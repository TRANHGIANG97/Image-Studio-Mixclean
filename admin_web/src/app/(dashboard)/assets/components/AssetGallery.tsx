...
export function AssetGallery({ assets, isLoading }: AssetGalleryProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between bg-slate-900/50 border border-slate-800/80 px-4 py-3.5 rounded-2xl text-xs backdrop-blur-sm transition-all duration-300">
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span className="font-semibold text-slate-300 hover:text-slate-100 transition-colors">
              Chọn tất cả ({assets.length})
            </span>
          </button>
          {selectedIds.length > 0 && (
            <span className="text-slate-500 font-medium bg-slate-950 px-2 py-0.5 rounded-md border border-slate-800/80">
        ...
        {assets.map((asset) => {
          ...
          return (
            <div
              ...
              className={`group relative bg-slate-900 border rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 select-none ${
                isSelected
                  ? 'border-indigo-500 shadow-lg shadow-indigo-600/10 bg-indigo-950/10'
                  : 'border-slate-800 hover:border-slate-700 hover:bg-slate-900/80 hover:shadow-md'
              }`}
            >
              ...
              <button
                ...
                className={`absolute top-2.5 left-2.5 z-20 w-6 h-6 rounded-lg flex items-center justify-center transition-all duration-300 border ${
                  isSelected
                    ...
                    : 'bg-slate-950/80 border-slate-800 text-slate-400 hover:border-slate-600 hover:text-white opacity-0 group-hover:opacity-100 scale-90'
                }`}
              >
                {isSelected ? (
                  <Check className="w-3.5 h-3.5 stroke-[3]" />
              ...
              {/* Action Overlay */}
              <div 
                className="absolute inset-0 bg-slate-950/40 opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center gap-3 z-10 backdrop-blur-[2px]"
              ...
              >
                <Button
                  ...
                  variant="secondary"
                  size="icon"
                  className="rounded-xl w-10 h-10 shadow-xl bg-slate-900/90 border border-slate-700/80 text-slate-300 hover:text-white hover:bg-slate-800 hover:border-slate-500 transition-all duration-200"
              ...
              {/* Thumbnail Preview */}
              <div
                className="aspect-square bg-slate-950 flex items-center justify-center relative overflow-hidden group-hover:scale-[1.02] transition-transform duration-500"
                style={{
                  backgroundImage:
              ...
              >
                {asset.mime_type?.startsWith('image/') ? (
                  ...
                  <div className="flex flex-col items-center justify-center text-slate-400">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-widest bg-slate-900 border border-slate-800 px-2 py-0.5 rounded-full text-slate-400">
              ...
              {/* Asset Info */}
              <div className="p-3 bg-slate-900/90 border-t border-slate-800/60 flex flex-col gap-1.5">
      ...
      {selectedIds.length > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-slate-900/95 border border-indigo-500/35 px-6 py-4 rounded-2xl shadow-2xl flex items-center justify-between gap-6 backdrop-blur-md animate-in fade-in slide-in-from-bottom-5 duration-300 min-w-[320px] md:min-w-[420px]">
          <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              className="text-slate-400 hover:text-white rounded-xl text-xs px-3.5 hover:bg-slate-800"
