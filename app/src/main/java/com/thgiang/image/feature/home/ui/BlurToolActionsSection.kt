...
fun BlurToolActionsSection(
    ...
    suspend fun buildBitmap(): Bitmap? = withContext(Dispatchers.IO) {
        ...
        val r = blurRadius
        if (r <= 0f) return@withContext original
        
        val blurred = blurBitmapForPortraitExport(original, r)
        if (blurred !== original && !original.isRecycled) {
            original.recycle()
        }
        blurred
    }
            ...
            onClick = {
                scope.launch {
                    ...
                    try {
                        ...
                        saveRepo.saveBitmap(
                            bmp,
                            fileName = "BLUR_${System.currentTimeMillis()}.png",
                            transparent = true
                        ).onSuccess {
                            Toast.makeText(
                                context,
